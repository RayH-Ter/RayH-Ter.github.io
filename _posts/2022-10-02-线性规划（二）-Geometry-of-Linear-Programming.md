...
2. **仿射集**、**锥集**、**凸集**
   ...
   仅是凸的，非仿射。但若把$\mathbf{A}$中每一列（每一列为$m$维的列向量）当作变量，将$\mathbf{x}$中每一元素看作系数，可以发现$P$也是一个锥。
...
5. **寻找极点**
   ...
   **观察：**从例子中发现，对于$\mathbf{A}\mathbf{x}=\mathbf{b}$的线性规划的可行域的顶点维数均为$n-m$维（假定变量数$n$大于线性方程组的个数$m$。因此可通过令$n-m$个变量为0，求解$m$维线性方程组，即可得到可能的极点。利用极点的代数性质可以把极点全部求出来，组合的可能有$n \choose n-m$种，然后排除不满足不等式条件的极点。
...
6. **求解！**
   ...
   1. 假设矩阵$\mathbf{A}$行满秩，则通过行交换和列交换可得下述公式。
   ...
   2. 如果$\mathbf{A}$不满秩那么：
      1. $\mathbf{Ax=b}$无解，因此可行域为空；
      2. 部分约束条件重复，此种情况将重复的条件剔除后$\mathbf{A}$重新变为满秩。
   3. 推论：
      1. 可行域种的一点$\mathbf{x}$是可行域的一个极点当且仅当$\mathbf{x}$是关于某个$\mathbf{B}$矩阵的basic feasible solution。
